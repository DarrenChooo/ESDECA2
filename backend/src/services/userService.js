...
module.exports.createUser = (fullname, email, password) => {
    createUserQuery = `INSERT INTO user ( fullname, email, user_password, role_id) VALUES (?,?,?,2) `
    return new Promise((resolve, reject) => {
        pool.getConnection((err, connection) => {
            if (err) {
                ...
                connection.query(createUserQuery, [fullname, email, password], (err, results) => {
                    if (err) {
                        reject(err);
                    } else {
                        resolve(results)
        ...
        return new Promise((resolve, reject) => {
            ...
            pool.getConnection((err, connection) => {
                if (err) {
                    ...
                    connection.query(`UPDATE user SET role_id =${sanitise(newRoleId)} WHERE user_id=${sanitise(recordId)}`, (err, rows) => {
                        if (err) {
                            reject(err);
                        } else {
                            resolve(rows);
...
module.exports.updateDesign = (recordId, title, description) => {
        return new Promise((resolve, reject) => {
            ...
            //to prepare the following code pattern which does not use callback technique (uses Promise technique)
            pool.getConnection((err, connection) => {
                reTitle = new RegExp(`^[\\w ]*$`)
                reDescription = new RegExp(`^[\\w ]*$`)
                if (reDescription.test(description) && reTitle.test(title) ) { 
                    if (err) {
                        console.log('Database connection error ', err);
                        resolve(err);
                    } else {
                        connection.query(`UPDATE file SET design_title ='${sanitise(title)}' , design_description='${sanitise(description)}' WHERE file_id=${recordId}`, (err, rows) => {
                            if (err) {
                                reject(err);
                            } else {
                                resolve(rows);
